...
def decode_latents_to_embeddings(
    latents: np.ndarray, rae_checkpoint: str, device: str = "cpu"
) -> np.ndarray:
    """
    ...
    if "model_state_dict" in ckpt:
        ...
        state_dict = ckpt  # checkpoint 直接就是 state dict
...
def decode_embeddings_to_sequences(
    ...
    results = decoder.decode(embeddings, return_distances=True, return_all_neighbors=True)
    ...
    for i, result in enumerate(results):
        ...
        record = {
            ...
            "length": nn["length"],
            # Top-5 近邻距离（用于评估新颖性）
            "nn_distances_top5": ",".join(
                [f"{n['distance']:.4f}" for n in result["neighbors"][:5]]
            ),
        }
...
def main(args):
    ...
    embeddings = decode_latents_to_embeddings(
        latents=latents, rae_checkpoint=args.rae_checkpoint, device=device
    )
    ...
    print(
        f"High novelty (dist > 0.10): {high_novelty} ({high_novelty / len(results_df) * 100:.1f}%)"
    )
    print(
        f"Medium novelty (0.05-0.10): {medium_novelty} ({medium_novelty / len(results_df) * 100:.1f}%)"
    )
...
if __name__ == "__main__":
    ...
    # 输入
    parser.add_argument("--latents", type=str, required=True, help="Path to latent vectors (.npy)")
    parser.add_argument(
        "--conditions", type=str, default=None, help="Path to conditions CSV (optional)"
    )

    # 模型
    parser.add_argument("--rae-checkpoint", type=str, required=True, help="Path to RAE checkpoint")
    ...
    parser.add_argument(
        "--metadata", type=str, default="data/embeddings/metadata.csv", help="Path to metadata CSV"
    )
    parser.add_argument(
    ...
    # 最近邻参数
    parser.add_argument(
        "--n-neighbors", type=int, default=5, help="Number of nearest neighbors to consider"
    )
    parser.add_argument(
        "--metric",
        type=str,
